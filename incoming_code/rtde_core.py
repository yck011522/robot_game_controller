...
def read_actual_joints(rtde_r: RTDEReceiveInterface) -> list[float]:
    joints = rtde_r.getActualQ()
    print("Actual joints (rad):", joints)
    return joints
...
def move_to_joint_target(
    ...
    print("Sending moveJ target (rad):", target_q)
    rtde_c.moveJ(target_q, speed, acceleration)
    print("moveJ finished.")


def stream_servoj(
    rtde_c: RTDEControlInterface,
    trajectory: Iterable[list[float]],
    frequency_hz: float,
    speed: float = 0.5,
    acceleration: float = 0.5,
    ...
    for q in trajectory:
        ...
        rtde_c.servoJ(q, speed, acceleration, dt, lookahead_time, gain)
        rtde_c.waitPeriod(cycle_start)
        sent += 1

    elapsed = time.monotonic() - start
    if elapsed > 0:
        print(f"servoJ sent {sent} setpoints in {elapsed:.3f}s (~{sent / elapsed:.1f} Hz)")
    else:
        print(f"servoJ sent {sent} setpoints")
